    ...
    private void chagePwd() throws IOException {
        ...
        if (user.checkPwd(userId, oldPwd)) {
            if (user.updataPwd(userId, newPwd)) {
                ...
                out.write("<script>alert('密码更改失败~~~');location.href='/Parking/Common/ChangePwd.jsp'</script>");
            }
        } else {
            out.write("<script>alert('原始密码错误~~~');location.href='/Parking/Common/ChangePwd.jsp'</script>");
        }
    }
